...
import { storPlansList, nowPlainStatus, storPlansProgress, StorMaterielInPlain } from '@/api/comapi'
...
export default {
  ...
  methods: {
    ...
    getStorPlansList (obj) {
      const that = Object.assign({}, {mode: this.$route.query.mode}, obj)
      storPlansList(that).then(res => {
    ...
    go01 (wd, mode) {
      if (this.isStart) {
        if (mode !== 4) {
          this.$router.push({path: '/warehousing/warehousing01', query: {wd: wd, mode: mode}})
        } else {
          MessageBox.alert('是否将当前作业单设置为已经完成？').then(action => {
            StorMaterielInPlain({mode: mode, id: JSON.parse(localStorage.getItem('workingId')) || ''}).then(res => {
              this.getStorPlansList({ pageNum: this.currentpageNum, pageSize: this.limit })
              this.getStorPlansProgress({mode: this.mode})
            })
          })
