...
export default {
  ...
  methods: {
    ...
    go01 (wd, mode) {
      if (this.isStart) {
        if (mode !== 3) {
          this.$router.push({path: '/warehousing/warehousing07', query: {wd: wd, mode: mode}})
        } else {
          MessageBox.alert('是否将当前作业单设置为已经完成？').then(action => {
            storTransportPlain({mode: mode, id: JSON.parse(localStorage.getItem('workingId')) || ''}).then(res => {
