...
import { storPlansList, nowPlainStatus, storPlansProgress, storMaterielMovePlain } from '@/api/comapi'
...
export default {
  ...
  methods: {
    ...
    go01 (wd, mode) {
      if (this.isStart) {
        if (mode !== 4) {
          this.$router.push({path: '/warehousing/warehousing04', query: {wd: wd, mode: mode}})
        } else {
          MessageBox.alert('是否将当前作业单设置为已经完成？').then(action => {
            storMaterielMovePlain({mode: mode, id: JSON.parse(localStorage.getItem('workingId')) || ''}).then(res => {
